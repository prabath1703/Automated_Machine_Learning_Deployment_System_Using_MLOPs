...
# ------------------ Config ------------------
DATASET_PATH = "data/student.csv"   # change to churn.csv if needed
...
# Make sure dataset has the correct columns
...
new_version = latest_versions[-1].version  # newest version
...
if EXPORT_ARTIFACTS:
    ...
    EXPORT_DIR = f"artifacts/best_model_{timestamp}"
    os.makedirs(EXPORT_DIR, exist_ok=True)
    mlflow.sklearn.save_model(best_model, path=EXPORT_DIR)
    print(f"Local export saved at: {EXPORT_DIR}")'''

# ... after training and selecting best_model

timestamp = time.strftime("%Y%m%d_%H%M%S")
EXPORT_DIR = f"artifacts/best_model_{timestamp}"

# Save best model to a timestamped folder
mlflow.sklearn.save_model(best_model, path=EXPORT_DIR)
print(f"Latest best model saved at {EXPORT_DIR}")
